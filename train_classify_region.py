...
def loadSeries(filelist, idx):
    ...
    y = label
    data = open(filename, 'r').readlines()[0].replace('\n','').split(':')[1].split(',')
...
if __name__ == '__main__':
    ...
    # Train
    model = ClassificationRNN2(place_dim = 10000, isAttention=config.isAttention)
    ...
    for epoch in range(config.training_epoch):
        ...
        for j, xs_batch, trainlen_batch, ys_batch in get_batch(xs, trainlen, ys):
            ...
            target = torch.FloatTensor(to_categorical(ys_batch, num_classes= 14)).cuda()
        ...
        if max_acc< acc:
            save_model_name = './trained_models/isAttention_{}_acc_{}'.format(config.isAttention, acc)
            max_acc = acc
