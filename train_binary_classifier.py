...
import os.path
import time
from ClassificiationRNN import ClassificationRNN2
import torch
import config
...
datasetDir = config.binaryDatasetDir
def loadSeries(filelist, idx):
    ...
    return name, label, FJZL2list.index(y), series
...
def evaluation(Y_test, Y_predict):
    ...
    return accuracy_score(Y_test, Y_predict), precision_recall_fscore_support(Y_test, Y_predict)[1][0]
...
if __name__ == '__main__':
    ...
    # Train
    model = ClassificationRNN2(place_dim = 180000,
    ...
    for epoch in range(config.training_epoch):
        ...
        for j, xs_batch, trainlen_batch, ys_batch in get_batch(xs, trainlen, ys):
            ...
            target = torch.FloatTensor(to_categorical(ys_batch, num_classes= 2)).cuda()
        ...
        acc,recall = evaluation(target, result)
        print("Overall Accuracy Score: {}".format(acc))

        print (prev_model)
        if max_acc< acc:
            save_model_name = './binary_models/{}_isAttention_{}_BiDir_{}_layer_{}_acc_{}_recall_{}'\
                .format(datasetDir.split('/')[-1], config.isAttention, config.biDirectional, config.number_layers, acc, recall)
            max_acc = acc
